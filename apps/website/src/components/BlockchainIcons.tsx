...
const icons: Record<string, ReactNode> = {
  ...
  wallet: (
    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path strokeLinecap="round" d="M3 10h18M7 15h1m4 0h1M5 6h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2z" />
    </svg>
  ),
  verify: (
    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function PanelIcon({ name }: { name: keyof typeof icons }) {
  ...
  return (
    <li className="feature-card">
      <div className="flex gap-3">
        <div className="info-panel-icon mt-0.5 h-9 w-9">{icons[icon]}</div>
        <div>
          <h3 className="font-display text-base font-semibold text-[var(--card-heading)]">{title}</h3>
          <p className="mt-2 text-sm leading-relaxed text-[var(--card-muted)]">{body}</p>
        </div>
      </div>
    </li>
  );
